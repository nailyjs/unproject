...
export class PluginDiscoveryService {
  ...
  private async loadPlugin(specifier: string): Promise<(options: any) => (UnProjectPlugin | Promise<UnProjectPlugin>) | undefined> {
    try {
  ...
  async loadPlugins(cache: boolean = true): Promise<UnProjectPlugin[]> {
    const plugins: UnProjectPlugin[] = []
